...
| FRA (destino/tamaño/uso) | `SHOW PARAMETER db_recovery_file_dest[_size]` ; `SELECT * FROM v$recovery_file_dest;`                   | `db_recovery_file_dest`     | Verifica el tamaño configurado ysu uso actual.                                                       |
...
1. Abre la terminal y entra a **SQL*Plus** como administrador de la instancia **orcl**:

   ```
   sqlplus / as sysdba
   ```
2. Mejora la legibilidad y comienza un **spool**:
...
1. Revisa `SGA_MAX_SIZE` (tope) y `SGA_TARGET` (autoajuste):
   ...
   ```
2. Observa los componentes dinámicos:
